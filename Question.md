...
### 问题4：在memory的设计过程中，第一次问答进行了循环生成了记忆，第二次问答过程生成记忆的时候，怎么控制不要和第一次生成的记忆点重复？
> 解决方案： 在调用extract_memories生成新的记忆的时候，会获取以前的记忆，生成一个prompt，告诉LLM这些记忆已经保存了，这样不会作为带总结的内容被总结。在调用LLM前保存了历史消息的快照，生成总结时就用这个快照，快照里面并没有注入以前的记忆